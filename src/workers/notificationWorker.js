...
const worker = new Worker('notification-stream', async (job) => {
    const trackingId = job.opts.jobId || job.id;
    console.log(`[Worker] 🛠️ Processing Job ID: ${trackingId}`);
    try {
        await new Promise((resolve) => setTimeout(resolve, 2000));
        if (Math.random() > 0.8) {
            throw new Error("External Provider Service Down (Simulated)");
        }
        await Notification.findOneAndUpdate({ jobId: trackingId }, { status: 'SUCCESS' });
        console.log(`[Worker] ✅ Successfully sent to ${job.data.recipient}`);
    }
    catch (err) {
        await Notification.findOneAndUpdate({ jobId: trackingId }, { status: 'FAILED', error: err.message });
...
worker.on('failed', (job, err) => {
    console.error(`[Worker] ⚠️ Job ${job?.id} has permanently failed: ${err.message}`);
});
